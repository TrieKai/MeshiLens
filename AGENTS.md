...
- **行程比較**：行程／餐次／候選店只存 `chrome.storage.local`；Maps 清單只有使用者主動按某一家「加入比較」時才執行該店單筆配對，不自動批量查 Tabelog。每餐最多 5 家。手機分享只輸出首選／備案與 Maps 連結，資料放在網址片段並由瀏覽器本機產生 QR Code，不寫入後端。
