...
- 擴充功能彈出視窗顯示目前版本，並提供啟用／停用開關、五種主題色與「這一餐吃哪家」入口；停用後立即移除評分卡並停止新的店家查詢
- 本機選店側邊工作區：幫這趟旅行建立餐次（例如「銀座週六晚餐」），從 Maps 把同一餐想比的店加進來，最多 5 家。新行程會先建立第一餐；只有你正看著待分類時，才會先放在那裡
- 以既有結構化資料提供「最值得專程去／附近快速吃／控制預算」三種透明排序；AI 未設定或失敗時仍可使用，並可選出首選與備案
- 將已決定的餐次產生精簡手機分享網址與本機 QR Code；行程資料只編碼在網址 `#` 後方，伺服器不接收也不保存內容
...
需要 Chrome／Edge 116 以上及 Python 3.12 以上。建議安裝 [uv](https://docs.astral.sh/uv/)，然後在本專案資料夾執行：
...
5. 點工具列上的 MeshiLens 圖示，將服務網址設為 `http://127.0.0.1:18765`，儲存後檢查服務狀態。
6. 按「這一餐吃哪家」，或直接從 Maps 搜尋結果／單店 MeshiLens 卡按「加入這一餐」。

## 本機選店與手機分享

這不是在比較兩條行程，而是決定**這一餐要吃哪家**：從 Maps 把同一餐的候選店加進側邊欄，並排看 Tabelog／Michelin／價位，再選首選與備案。

資料只存於瀏覽器的 `chrome.storage.local`，不需要登入或雲端帳號。每個行程可建立最多 12 個餐次；尚未分類的候選店最多 20 家，每個實際比較餐次最多 5 家。從 Maps 搜尋清單加入時，只有使用者明確點擊的那家店會呼叫既有單店配對 API，不會將整張清單送去 Tabelog 搜尋。

排序使用 Tabelog 評分與評論數、午／晚餐價位、店家與集合點的直線距離、配對信心，以及 Michelin／百名店等既有結構化欄位。內部規則只用於排序，畫面不顯示神祕綜合分數；資料缺失會標為未知，指定日期或餐期時也只提醒再次確認營業時間，不會自動宣判休業。AI 不參與核心排名。

分享時只輸出各餐已選定的首選、備案與 Maps 連結。資料經清理後編碼在 `landing/share.html#...` 的網址片段；瀏覽器不會把片段送進 HTTP 請求，QR Code 也完全在本機產生。分享網址本身持有行程內容，仍應只傳給預期的同行者。
...
- 為了在 Google Maps 搜尋清單顯示 Michelin 快照徽章，擴充功能只會讀取目前畫面可見店家卡的名稱、地址與連結並批次查詢快照；清單不會自動發出 Tabelog 搜尋。只有使用者按下某一家「加入這一餐」時，才會對該店執行一次既有單店配對；不讀取 Maps 評論。
- 行程與比較條件只保存在瀏覽器本機。手機分享頁的精簡資料位於網址片段且不寫入後端；任何持有完整分享網址的人都能看到首選／備案，因此應將其視為可轉傳的行程資料。
...
extension/planner.js       行程、餐次、候選店與透明排序領域邏輯
extension/planner.html     「這一餐吃哪家」選店側邊工作區
