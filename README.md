...
- 本機行程比較側邊工作區：建立多個命名行程與「銀座週六晚餐」等用餐場合，從 Maps 搜尋結果或單店卡主動加入候選店；未指定餐次時先放入待分類，每餐最多比較 5 家
...
行程比較資料只存於瀏覽器的 `chrome.storage.local`，不需要登入或雲端帳號。每個行程可建立最多 12 個用餐場合；尚未分類的候選店最多 20 家，每個實際比較餐次最多 5 家。從 Maps 搜尋清單加入時，只有使用者明確點擊的那家店會呼叫既有單店配對 API，不會將整張清單送去 Tabelog 搜尋。
