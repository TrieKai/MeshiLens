...
- 配對完成後，以一次受限的 Tabelog 搜尋頁尋找最多三家相似且附近的店；同站優先，不同站時必須能確認同一行政區或地址區域，無法確認位置就不推薦；再依料理、價位與搜尋卡片的評分／評論數排序，不會自動讀取推薦店詳情或評論；點擊推薦卡會以店名與地址在 Google Maps 開啟
- 相似店家的常見 Tabelog 料理標籤會轉為繁中；對照表集中於 `src/meshi_lens/data/tabelog-genres-zh-hant.json`，未知標籤保留原文以避免猜測翻譯
- 相似店家優先使用同站或同一行政區；候選卡片缺少地點欄位時，僅在 Tabelog 已明確以原店車站搜尋時採用該搜尋範圍，避免跨區誤薦
