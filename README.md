...
- 配對完成後，以受限的 Tabelog 店家專屬「附近店家」頁尋找最多三家相似且附近的店；已知料理類別直接使用官方附近分類頁，未知類別先讀取官方分類連結、確認有對應分類時才追加讀取該分類頁，避免泛附近餐廳被誤當相似店家；不會自動讀取推薦店詳情或評論。使用者點擊推薦卡時，才低頻讀取該店的 Tabelog 地圖地址並以完整地址開啟 Google Maps，避免同名誤導。
