...
- 配對完成後，以一次受限的 Tabelog 店家專屬「附近店家」頁尋找最多三家相似且附近的店；已知料理類別才使用其官方附近分類頁，其他類別必須仍有同料理、價位等明確訊號才推薦，避免把泛附近餐廳誤當相似店家；不會自動讀取推薦店詳情或評論。使用者點擊推薦卡時，才低頻讀取該店的 Tabelog 地圖地址並以完整地址開啟 Google Maps，避免同名誤導。
- 相似店家的常見 Tabelog 料理標籤會轉為繁中；對照表集中於 `src/meshi_lens/data/tabelog-genres-zh-hant.json`，未知標籤保留原文以避免猜測翻譯。合格候選最多保留 6 家，預設顯示 3 家，可直接依推薦度、Tabelog 評分、評論數排序或按料理篩選，不會增加 Tabelog 請求。
...
- 相似店家只在已配對店家後進行一次附近頁查詢，最多處理前 20 筆並快取；不批量掃描、不抓推薦店詳情或評論，也不在推薦搜尋失敗時擴大重試或改用外部搜尋。只有使用者明確點擊一張推薦卡時，才會讀取一次該店的地圖地址並快取。
