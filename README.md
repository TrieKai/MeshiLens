...
- 擴充功能彈出視窗顯示目前版本，並提供啟用／停用開關、五種主題色與「開啟行程比較」入口；停用後立即移除評分卡並停止新的店家查詢
- 本機行程比較側邊工作區：建立多個命名行程與「銀座週六晚餐」等餐次，從 Maps 搜尋結果或單店卡主動加入候選店；新行程會先建立第一餐，之後加入的店會進目前餐次。只有你正看著待分類時，才會先放在那裡；每餐最多比較 5 家
...
6. 按「開啟行程比較」，或直接從 Maps 搜尋結果／單店 MeshiLens 卡按「加入比較」。

## 本機行程比較與手機分享

行程比較資料只存於瀏覽器的 `chrome.storage.local`，不需要登入或雲端帳號。每個行程可建立最多 12 個餐次；尚未分類的候選店最多 20 家，每個實際比較餐次最多 5 家。從 Maps 搜尋清單加入時，只有使用者明確點擊的那家店會呼叫既有單店配對 API，不會將整張清單送去 Tabelog 搜尋。
...
- 為了在 Google Maps 搜尋清單顯示 Michelin 快照徽章，擴充功能只會讀取目前畫面可見店家卡的名稱、地址與連結並批次查詢快照；清單不會自動發出 Tabelog 搜尋。只有使用者按下某一家「加入比較」時，才會對該店執行一次既有單店配對；不讀取 Maps 評論。
