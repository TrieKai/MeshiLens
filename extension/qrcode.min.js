 ...
 * Adapted from the qrcode-terminal vendored implementation for browser canvas rendering.
 */
(()=>{
  const modules = {
...
module.exports = QRCode;
  ...
  const QRCodeCore = localRequire("index");
  const CorrectLevel = localRequire("./QRErrorCorrectLevel");
  function QRCode(container, options = {}) {
    const text = String(options.text || "");
    const code = new QRCodeCore(-1, options.correctLevel ?? CorrectLevel.M);
    code.addData(text);
    code.make();
    const quiet = 4;
    const modulesCount = code.getModuleCount();
    const cssSize = Math.max(180, Number(options.width) || 288);
    const dpr = Math.max(1, Number(globalThis.devicePixelRatio) || 1);
    const modulesWithQuiet = modulesCount + quiet * 2;
    const cell = Math.max(4, Math.round((cssSize * dpr) / modulesWithQuiet));
    const size = cell * modulesWithQuiet;
    ...
    canvas.setAttribute("aria-label", "手機分享 QR Code");
    const context = canvas.getContext("2d");
    context.fillStyle = options.colorLight || "#fff";
    context.fillRect(0, 0, size, size);
    context.fillStyle = options.colorDark || "#000";
    for (let row = 0; row < modulesCount; row += 1) {
      for (let column = 0; column < modulesCount; column += 1) {
